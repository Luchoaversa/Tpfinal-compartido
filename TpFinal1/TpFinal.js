...
function mousePressed() {
  // INICIO
  if (escena === -1) {
    if (mouseDentro(width/2 - 75, height/2 - 20, 150, 40)) escena = 0; // VA A PANTALLA 1 (índice 0)
    else if (mouseDentro(width/2 - 75, height/2 + 40, 150, 40)) escena = 99; // VA A CRÉDITOS
  }
  // Créditos/Finales
  else if (escena === 99 || (escena >= 100 && escena <= 102)) {
    if (mouseDentro(width/2 - 75, height - 80, 150, 40)) escena = -1; // VA A INICIO
  }
  // Pantallas de juego
  else if (escena >= 0 && escena < totalImages) {
    if (esBifurcacion(escena)) {
      if (mouseDentro(width/2 - 75, 300, 150, 40)) escena = destinoA(escena);
      else if (mouseDentro(width/2 - 75, 360, 150, 40)) escena = destinoB(escena);
    }
